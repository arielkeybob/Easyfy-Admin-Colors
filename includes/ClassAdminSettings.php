...
class ClassAdminSettings {
    ...
    public function color_picker_callback($args) {
        ...
        // Verifique se o nome está correto e inclui os brackets
        echo '<input type="text" id="' . esc_attr($args['id']) . '" name="easyfy_admin_colors_settings[' . esc_attr($args['id']) . ']" value="' . esc_attr($color) . '" class="color-picker" data-default-color="' . esc_attr($args['default']) . '" />';
    }
    

    public function validate_inputs($inputs) {
        // Log before validation
        error_log('Receiving the following input before validation: ' . print_r($inputs, true));
        
        if (!is_array($inputs)) {
            $inputs = [];
            error_log('Input was not an array. Resetting to an empty array.');
        }
    
        foreach ($inputs as $key => $value) {
            $sanitized_value = sanitize_hex_color($value);
            error_log("Original: {$value}, Sanitized: {$sanitized_value}");
            $inputs[$key] = $sanitized_value;
        }
    
        // Log after validation
        error_log('Processed the following input after validation: ' . print_r($inputs, true));
